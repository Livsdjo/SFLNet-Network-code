...
def build_graph(xs,config):
    ...
    if config['use_sparse']:
        ...
        # eig_val,eig_vec=eigsh(laplacian,laplacian.shape[0]-1,which='SA')
        # eig_val1,eig_vec1=eigsh(laplacian,64,which='LM')
        # eig_val = np.concatenate((eig_val, eig_val1), 0)
        # eig_vec = np.concatenate((eig_vec, eig_vec1), 1)
        # print("维度", eig_val.shape, eig_vec.shape, laplacian.shape[0]-1)
    else:
        degree = np.sum(adj, 1)
        laplacian = -adj
