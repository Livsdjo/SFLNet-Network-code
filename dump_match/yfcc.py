...
parser.add_argument('--dump_dir', type=str, default='/home/featurize/data/data_dump6/',
...
if __name__ == "__main__":
    ...
    """
    with open('/home/featurize/work/MS2DG-code/MS2DGNET/dump_match/yfcc_train.txt','r') as ofp:
        train_seqs = ofp.read().split('\n')
    if len(train_seqs[-1]) == 0:
        del train_seqs[-1]
    print('train seq len '+str(len(train_seqs)))
    yfcc_tr_va = Dataset(config.raw_data_path+'yfcc100m/', config.dump_dir, 'yfcc-'+config.desc_name+'-test-known.hdf5', \
