...
class Sequence(object):
    ...
    def make_xy(self, ii, jj):
        ...
        # return xs, ys, dR, dt, ratio_test, mutual_nearest, cx1, cy1, f1, cx2, cy2, f2, eig_data
    ...
    def compute_eig(self,):
        for seq_name in self.train_seq_names:
            if seq_name != "colosseum_exterior":
                continue
            ...
            for pair_id in tqdm(train_dataset.pair_ids):
                ...
                if id0 == "1070" or id0 == "1240":
                    print(11111111)
                    img = train_dataset.get_image(id0)
                    kps0, desc0 = self.extractor(img)
                    img = train_dataset.get_image(id1)
                    kps1, desc1 = self.extractor(img)

                    kps0 = kps0[:, :2]
                    kps1 = kps1[:, :2]
                    K0 = train_dataset.get_K(id0)
                    K1 = train_dataset.get_K(id1)
                elif id1 == "1070" or id1 == "1240":
                    print(22222222)
                    img = train_dataset.get_image(id0)
                    kps0, desc0 = self.extractor(img)
                    img = train_dataset.get_image(id1)
                ...
                print("预处理", kps0[matches[:, 0]], K0)


                x0 = self.hpts_to_pts(self.pts_to_hpts(kps0[matches[:, 0]]) @ np.linalg.inv(K0).T)
                x1 = self.hpts_to_pts(self.pts_to_hpts(kps1[matches[:, 1]]) @ np.linalg.inv(K1).T)
                xs = np.concatenate([x0,x1],1)

                print("计算特征值", xs.shape, xs)
    ...
    def dump_datasets(self):
        ...
        # var_name = ['xs', 'ys', 'Rs', 'ts', 'ratios', 'mutuals', 'cx1s', 'cy1s', 'f1s', 'cx2s', 'cy2s', 'f2s', 'eig_data']
        res_dict = {}
